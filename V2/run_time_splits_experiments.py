...
def run_all_splits(
    ...
    for split_idx, (train_path, test_path) in enumerate(pairs, start=1):
        ...
        # Separate ID/meta columns from features
        id_cols = [
            "movie_title",
            "title_year",
            "genres",
        ...
        id_cols = [c for c in id_cols if c in train_df.columns]
    ...
    if "budget" in meta_all.columns:
        ...
        plot_group_residuals_by_feature(
            y_true_all,
            y_pred_for_bins,
            feature=meta_all["budget"],
            feature_name=f"budget_{model_name_for_bins}",
        )
    ...
    if build_learning_curves:
        ...
        id_cols = [c for c in id_cols if c in last_train_df.columns]
